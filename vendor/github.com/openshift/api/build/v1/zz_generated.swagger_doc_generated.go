...
var map_BuildVolume = map[string]string{
	...
	"name":   "name is a unique identifier for this BuildVolume. It must conform to the Kubernetes DNS label standard and be unique within the pod. It cannot collide with volumes that are always added by the build controller. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names",
	"source": "source represents the location and type of the mounted volume.",
	"mounts": "mounts represents the location and read/write status of the volume in the buildah container",
}
...
var map_BuildVolumeMount = map[string]string{
	...
	"destinationPath": "destinationPath is the path within the buildah runtime environment at which the volume should be mounted. Must not contain ':' and can not collide with a destination path generated by the builder process.",
	"readOnly":        "readOnly denotes whether the volume should be mounted as readonly or not The volume is mounted read-only if true, read-write otherwise (false or unspecified). Defaults to false.",
}
...
var map_BuildVolumeSource = map[string]string{
	...
	"type":      "type is the BuildVolumeSourceType for the volume source. Type must match the populated volume source, and if not specified will be inferred on admission.",
...
var map_DockerBuildStrategy = map[string]string{
	...
	"volumes":                 "volumes is a list of input volumes that can be mounted into buildah's runtime environment. Only a subset of Kubernetes Volume sources are supported by builds. More info: https://kubernetes.io/docs/concepts/storage/volumes",
}
