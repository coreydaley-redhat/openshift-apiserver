...
type BuildVolume struct {
	...
	// It cannot collide with volumes that are always added by the build controller.
...
type BuildVolumeMount struct {
	...
	// readOnly denotes whether the volume should be mounted as readonly or not
	// The volume is mounted read-only if true, read-write otherwise (false or unspecified).
	// Defaults to false.
	// +optional
